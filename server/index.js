...
function authentication(req, res, next) {
  ...
  if (
    req.originalUrl.includes("/getImgQrCode/") ||
    req.originalUrl.includes("/2c2p")
  ) {
    next();
  } else {
    if (!authheader) {
    ...
    if (
      user == encrypt.EncodeKey("administrator") &&
      pass == encrypt.EncodeKey("asujimuser")
    ) {
      // If Authorized user
      next();
    } else {
...
db.sequelize.sync().then(() => {
  app.listen(port, () => {
    ...
    function fetchMostRecentData() {
      axios
        .get("https://undefined.ddns.net/mbkserver/order/orderHD/getOrderOver48Hour")
        .then((response) => {
