...
const PurchaseOrder = ({ props }) => {
  ...
  const thClass =
    "px-2  py-4 text-sm  border-l-0 border-r-0 whitespace-nowrap text-left text-blueGray-500 ";
  const tdClass =
    "border-t-0  py-3 pl-2 align-middle border-l-0 border-r-0 py-1 text-sm whitespace-normal";
  const tdSpan = "text-gray-mbk hover:text-gray-mbk ";

  const thClassFooter =
    "px-2  py-2 text-sm  border-l-0 border-r-0 whitespace-nowrap text-left text-blueGray-500 ";
  const tdClassFooter =
    "border-t-0  py-1 pl-2 align-middle border-l-0 border-r-0 py-1 text-12 whitespace-normal";
        ...
        <table className="items-center w-full ">
          <thead>
            ...
            >
              ...
              <th className={thClass + " w-6/12"}>สินค้า</th>
              <th className={thClass + " text-right"}>จำนวน</th>
              <th className={thClass + " text-right pr-4"}>ราคาต่อหน่วย</th>
            </tr>
          </thead>
          <tbody style={{ borderBottom: "1px solid  #e5e7eb" }}>
            {orderDT.map(function (value, key) {
              return (
                <tr key={key}>
                  ...
                  <td className={tdClass + " w-full"}>
                    <div className="flex">
                      ...
                      <div className="flex flex-col">
                        <div className={tdSpan}>{value.productName}</div>
                  ...
                  <td className={tdClass + " text-right"}>
                    <span className={tdSpan}>{value.amount}</span>
                  </td>
                  {value.discount > 0 ? (
                    <td
                      className={tdClass + " margin-auto-t-b text-right pr-4"}
                    >
                      <div className="flex flex-col">
                        <div>
                          ฿
                          {(
                            parseFloat(value.price) - parseFloat(value.discount)
                          )
                            .toFixed(2)
                            .toLocaleString("en")}
                        </div>
          ...
          <tfoot className="mt-2">
            <tr>
              <td className={thClassFooter}>
                <span className={tdSpanFooter}></span>
              </td>
              <td className={thClassFooter}>
                <span className={tdSpanFooter}></span>
              </td>
              ...
              <td className={tdClassFooter + " text-right font-bold text-12"}>
                <span className={" pr-4"}>{fn.formatMoney(sumPrice)}</span>
              </td>
            </tr>
            <tr>
              <td className={thClassFooter}>
              ...
              <td className={tdClassFooter + " text-right"}>ส่วนลดร้านค้า</td>
              <td className={tdClassFooter + " text-right"}>
                <div
                  className={
                    "text-12  border-l-0 border-r-0 whitespace-nowrap text-left  pr-4" +
                    (orderHD.discountStorePromotion > 0 ? " text-gold-mbk" : "")
                  }
                  style={{ textAlign: "end" }}
                >
                  {orderHD.discountStorePromotion > 0
                    ? "-" + fn.formatMoney(orderHD.discountStorePromotion)
                    : fn.formatMoney(0)}
                </div>
              </td>
            </tr>
            ...
            <tr>
              ...
              <td className={tdClassFooter + " text-right "}>ค่าจัดส่ง</td>
              <td className={tdClassFooter + " text-right "}>
                <div
                  className={
                    "text-12  border-l-0 border-r-0 whitespace-nowrap text-left  pr-4"
            ...
            <tr>
              ...
              <td className={tdClassFooter + " text-right "}>
                <div
                  className={
                    "text-12  border-l-0 border-r-0 whitespace-nowrap text-left  pr-4" +
                    (orderHD.discountDelivery > 0 ? " text-gold-mbk" : "")
                  }
                  style={{ textAlign: "end" }}
                >
                  {orderHD.discountDelivery > 0
                    ? "-" +
                      fn.formatMoney(
                        orderHD.deliveryCost - orderHD.discountDelivery
                      )
                    : fn.formatMoney(0)}
                </div>
              </td>
            ...
            <tr>
              ...
              <td className={tdClassFooter + " text-right border-b"}>
                <div
                  className={
                    "text-12  border-l-0 border-r-0 whitespace-nowrap text-left  pr-4" +
                    (orderHD.discountCoupon > 0 ? " text-gold-mbk " : "")
                  }
                  style={{ textAlign: "end" }}
                >
                  {orderHD.discountCoupon > 0
                    ? "-" + fn.formatMoney(orderHD.discountCoupon)
                    : fn.formatMoney(0)}
                </div>
              </td>
            </tr>
