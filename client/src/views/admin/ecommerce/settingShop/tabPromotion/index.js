...
const Promotion = () => {
  ...
  const [errorPercentDiscountAmount, setErrorPercentDiscountAmount] =
    useState(false);
      ...
      setListPromotion(
        listSearch.filter(
          (x) =>
            ...
            (x.condition === "discount"
              ? "ส่วนลด"
              : x.condition === "percentDiscount"
              ? "% ส่วนลด"
              : "สินค้าสมนาคุณ"
            )
              .toLowerCase()
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      ...
      buy: yup
        .number()
        .required("* กรุณากรอก ซื้อครบ")
        .min(0.01, " ซื้อครบต้องมากกว่า 0"),
      discount: yup.number().when("condition", {
      ...
      percentDiscount: yup.number().when("condition", {
        is: (condition) => condition == 2,
        then: yup
          .number()
          .required("* กรุณากรอก %ส่วนลด")
          .min(0.01, " %ส่วนลดต้องมากกว่า 0"),
      }),
      percentDiscountAmount: yup.number().when("condition", {
        is: (condition) => condition == 2,
        then: yup
          .number()
          .required("* กรุณากรอก ส่วนลดสูงสุด")
          .min(0.01, " ส่วนลดสูงสุดต้องมากกว่า 0"),
      }),
